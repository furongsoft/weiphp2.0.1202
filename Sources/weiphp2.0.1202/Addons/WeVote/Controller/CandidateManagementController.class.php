...
ORDER BY count DESC LIMIT %d, %d";
        ...
        $this->SQL_QUERY_CANDIDATES_COUNT =
            "SELECT COUNT(id) AS count FROM " . C('DB_PREFIX') . "wevote_candidates WHERE (action_id = %d) AND (status = 1)";

        // 查询候选人SQL语句
        $this->SQL_QUERY_CANDIDATE = "SELECT * FROM " . C('DB_PREFIX') . "wevote_candidates WHERE (id = %d)";

        // 更新候选人SQL语句
        $this->SQL_UPDATE_CANDIDATE =
            "UPDATE " . C('DB_PREFIX') . "wevote_candidates SET title='%s', comment='%s', announce='%s', parent='%s', tel='%s', address='%s', cell='%s', picture='%s' WHERE (id = %d)";
    ...
    public function Index()
    {
        ...
        $fields[] = array('title' => '宣言', 'name' => 'announce', 'type' => 'string');
        $fields[] = array('title' => '家长姓名', 'name' => 'parent', 'type' => 'string');
        $fields[] = array('title' => '联系方式', 'name' => 'tel', 'type' => 'string');
        $fields[] = array('title' => '住宅名称', 'name' => 'address', 'type' => 'string');
        $fields[] = array('title' => '楼层单元', 'name' => 'cell', 'type' => 'string');
        $fields[] = array('title' => '图片', 'name' => 'picture', 'type' => 'picture');
        $fields[] = array('title' => '票数', 'name' => 'count', 'type' => 'num');

        // 设置参数
        $this->assign('list_fields', $fields);
        $this->assign('list_data', $data);
        $this->assign('remove_url', addons_url('WeVote:/CandidateManagement/RemoveAction', array('action_id' => $this->mActionId)));
        ...
        $this->display(ONETHINK_ADDON_PATH . 'WeVote/View/default/WeVote/Admin/CandidateManagement/List.html');
    }
    ...
    public function Edit()
    {
        ...
        $this->assign('post_url', addons_url('WeVote:/CandidateManagement/EditAction'));

        $this->display(ONETHINK_ADDON_PATH . 'WeVote/View/default/WeVote/Admin/CandidateManagement/Edit.html');
    }
    ...
    public function EditAction()
    {
        ...
        if ($result !== false)
            return $this->success('修改成功', addons_url('WeVote:/CandidateManagement/Index', array('action_id' => $this->mActionId)));
        else
    ...
    public function RemoveAction()
    {
        ...
        if (($ret1 !== false) && ($ret2 !== false)) {
            $model->commit();
            return $this->success('删除成功', addons_url('WeVote:/CandidateManagement/Index', array('action_id' => $this->mActionId)));
        } else {
            $model->rollback();
