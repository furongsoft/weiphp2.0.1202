 ...
 *  @author https://github.com/dodgepudding/wechat-php-sdk
 *
 */
namespace Addons\Abcinfosend\Api;
class Wechat
{
	...
	*/
	...
	public function uploadnews($data){
		...
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post('https://api.weixin.qq.com/cgi-bin/media/uploadnews?access_token='.$this->access_token,self::json_encode($data));
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
	 ...
	 * @return boolean/array
	 */
	public function sendall($group_id,$media_id,$msgtype='image'){
		$senddata=array(
				'filter'=>array(
						'group_id'=>$group_id
					),
		...
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post('https://api.weixin.qq.com/cgi-bin/message/mass/sendall?access_token='.$this->access_token,self::json_encode($senddata));
		if ($result)
		{
			$json = json_decode($result,true);
			return $json;
	...
	public function getUserGroupid($openid){
		...
		$result = $this->http_post('https://api.weixin.qq.com/cgi-bin/groups/getid?access_token='.$this->access_token,self::json_encode($data));
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
