 ...
 * @author 麦当苗儿 <zuojiazi@vip.qq.com>
 */
function addons_url($url, $param = array(), $suffix=true)
{
    ...
    return U('Home/Addons/execute', $params, $suffix);
...
//ALEX[[[
function GetListField($field, $data)
{
    $name = $field['name'];
    $type = $field['type'];
    if (strcasecmp($type, 'picture') == 0)
        return get_img_html($data[$name]);
    else
        return $data[$name];
}
...
function get_jsapi_ticket($token = '')
{
    ...
    $url = "https://api.weixin.qq.com/cgi-bin/ticket/getticket?type=jsapi&access_token=" . get_access_token($token);
    $tempArr = json_decode(file_get_contents($url), TRUE);
    if (@array_key_exists('ticket', $tempArr)) {
        S($key, $tempArr['ticket'], 7200 - 200);
...
function GetSignPackage($token = '')
{
    ...
    $jsapiTicket = get_jsapi_ticket($token);
